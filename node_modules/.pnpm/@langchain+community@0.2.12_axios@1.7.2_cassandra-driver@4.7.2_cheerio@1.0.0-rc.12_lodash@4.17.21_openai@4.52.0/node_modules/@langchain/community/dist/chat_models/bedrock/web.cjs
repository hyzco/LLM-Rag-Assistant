...
class BedrockChat extends chat_models_1.BaseChatModel {
    ...
    async _signedFetch(messages, options, fields) {
        ...
        const inputBody = this.usesMessagesApi
            ? index_js_1.BedrockLLMInputOutputAdapter.prepareMessagesInput(provider, messages, this.maxTokens, this.temperature, options.stop ?? this.stopSequences, this.modelKwargs, this.guardrailConfig, this._anthropicTools)
            : index_js_1.BedrockLLMInputOutputAdapter.prepareInput(provider, convertMessagesToPromptAnthropic(messages), this.maxTokens, this.temperature, options.stop ?? this.stopSequences, this.modelKwargs, fields.bedrockMethod, this.guardrailConfig);
    ...
    bindTools(tools, _kwargs) {
        ...
        this._anthropicTools = tools.map((tool) => {
            if ((0, function_calling_1.isStructuredTool)(tool)) {
                return {
                    name: tool.name,
